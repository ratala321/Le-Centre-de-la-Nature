...
static func effectuer_transition_basique_camera_3d(initiale : Camera3D,
	...
	if not _transition_en_cours:
		...
		await _deplacer_camera_transition_vers_destination(
			camera_de_transition, destination, duree_transition)
